...
// https://scrollmagic.io/docs/index.html -> usage본문 복사및 필요한부분 정리해서 사용

// swiper 사용
const swiper = new Swiper('.project .swiper', {
  // 슬라이드 옵션 지정 
  direction: 'horizontal', //수평 슬라이드
  loop: true, //반복 재생 여부, 1->2->3->다시1 
  autoplay: {
    delay:3000,
  },
...
window.addEventListener('scroll', function(){
 ...
 //페이지 스크롤 위치가 500px을 넘으면 요소를 보이고 500px을 넘지 않으면 요소 숨기기
  ...
  if(window.scrollY>500){
    ...
    visualSpanEls.forEach(function (visualSpan){ visualSpan.classList.remove('animate-flash');}) 
  }
  else {
